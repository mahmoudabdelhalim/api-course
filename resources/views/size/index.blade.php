					...
					<table id="courseEval" class="dattable table table-striped thead-dark  w-100">
						...
						@foreach($rows as $index => $row)
							...
							<td>{{ $index +1 }}</td>
									  ...
									  <td>
                                      <a href="{{ route('size.edit', $row->id) }}" class="btn btn-info d-inline-block" 
                                      >edit</a>
                                        <a href="#" onclick="destroy('this Size','{{$row->id}}')" class="btn d-inline-block btn-danger">delete</a>
                                         <form id="delete_{{$row->id}}" action="{{ route('size.destroy', $row->id) }}"  method="POST" style="display: none;">
									@csrf
									@method('DELETE')
